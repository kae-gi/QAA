...
zcat $file1 | awk 'NR%4==2 {print length ()}'| sort | uniq -c | sort -n > ./read_length_dist/21.R1.read_length_dist.txt

echo "DONE WITH 21_R1"
echo ""

zcat $file2 | awk 'NR%4==2 {print length ()}'| sort | uniq -c | sort -n > ./read_length_dist/21.R2.read_length_dist.txt

echo "DONE WITH 21_R2"
echo ""

zcat $file3 | awk 'NR%4==2 {print length ()}'| sort | uniq -c | sort -n > ./read_length_dist/34.R1.read_length_dist.txt

echo "DONE WITH 34_R1"
echo ""

zcat $file4 | awk 'NR%4==2 {print length ()}'| sort | uniq -c | sort -n > ./read_length_dist/34.R2.read_length_dist.txt
